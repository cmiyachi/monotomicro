# UdaConnect
## Overview
This is the README given in the starter code.  Please see the end of theis file for the section "Microservices."
...
The following directions can be used to deploy this project.  All of the imgages are on dockerhub. 

## Deployment

1.  Get the DB up and running:
* `kubectl apply -f deployment/db-configmap.yaml` - Set up environment variables 
* `kubectl apply -f deployment/db-secret.yaml` - Set up secrets fo
* `kubectl apply -f deployment/postgres.yaml` - Set up a Postgres database 
...
2. Get the app up and running (these are similar to the monolith)
* `kubectl apply -f deployment/udaconnect-api.yaml`
* `kubectl apply -f deployment/udaconnect-app.yaml` 
3. Install the connection service
 * `kubectl apply -f deployment/connection-svc.yaml` 
*  Ditto for the location service: `kubectl apply -f deployment/location-svc.yaml`
* and the person service `kubectl apply -f deployment/person-svc.yaml` -
4.  Install Kafka by following the below steps:
...
5. After this, deploy the consumer and producers of location events
*  `kubectl apply -f deployment/person-location-consumer-svc*.yaml`
*  `kubectl apply -f deployment/person-location-producer-svc*.yaml`

## Testing
Use the postman project to test the endpoints of the project.  Each one returns data and/or 20X status code when deployed. 